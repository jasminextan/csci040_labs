...
'''
import json
paths = ['trump tweets\condensed_2009.json\condensed_2009.json', 'trump tweets\condensed_2010.json\condensed_2010.json', 'trump tweets\condensed_2011.json\condensed_2011.json',
'trump tweets\condensed_2012.json\condensed_2012.json','trump tweets\condensed_2013.json\condensed_2013.json','trump tweets\condensed_2014.json\condensed_2014.json','trump tweets\condensed_2015.json\condensed_2015.json',
'trump tweets\condensed_2016.json\condensed_2016.json','trump tweets\condensed_2017.json\condensed_2017.json','trump tweets\condensed_2018.json\condensed_2018.json']
tweets = []
for path in paths:
...
#i just picked your words so that i could verify that my data was correct

appearsintweet = {"daca":0, "fake news":0, "mainstream media":0, "mexico":0, "obama":0, "russia":0, "trump":0, "wall": 0}

for tweet in tweets:
    if 'daca' in tweet['text'].lower():
        appearsintweet["daca"] +=1
    if 'fake news' in tweet['text'].lower():
        appearsintweet["fake news"] +=1
    if 'mainstream media' in tweet['text'].lower():
        appearsintweet["mainstream media"] += 1
    if 'mexico' in tweet['text'].lower():
        appearsintweet['mexico'] +=1
    if  'obama' in tweet['text'].lower():
        appearsintweet['obama'] +=1
    if  'russia' in tweet['text'].lower():
        appearsintweet['russia'] +=1
    if  'trump' in tweet['text'].lower():
        appearsintweet['trump'] +=1
    if  'wall' in tweet['text'].lower():
        appearsintweet['wall'] +=1

print("```")
print('the total number of tweets =',len(tweets))
print("```")
print('the number of times each of these phrases appears:')
for i in appearsintweet:
    print(f"{i:>20} : {appearsintweet[i]}")
print("```")
print('percentage of tweets using phrase:')
for i in appearsintweet:
    print(f"{i:>20} : {(appearsintweet[i]/len(tweets) * 100):05.2f}")
print("```")
